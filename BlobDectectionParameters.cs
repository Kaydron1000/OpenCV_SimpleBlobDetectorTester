...
namespace WpfApp1
{
    public class BlobDectectionParameters : INotifyPropertyChanged
    {
        ...
        public BlobDectectionParameters(MainViewModel ParentView)
        {
        ...
        private void BlobDectectionParameters_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (_ParentView?.ImgMat != null)
            {
                if (sender is BlobDectectionParameters)
                {
                    ...
                    var kep = _Detector.Detect(_ParentView.ImgMat);
                    Mat im_with_keypoints = new Mat();
                    Cv2.DrawKeypoints(image: _ParentView.ImgMat, kep, im_with_keypoints, Scalar.Beige, DrawMatchesFlags.DrawRichKeypoints);
                }
            }
        }
